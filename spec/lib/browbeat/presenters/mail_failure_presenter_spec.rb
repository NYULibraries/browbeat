...
describe Browbeat::Presenters::MailFailurePresenter do
  ...
  describe "instance methods" do
    ...
    describe "screenshots_url" do
      subject{ presenter.screenshots_url }

      context "with jenkins_build_tag" do
        let(:build_tag){ "jenkins-Some Thing-2000" }
        before{ allow(presenter).to receive(:jenkins_build_tag).and_return build_tag }

        it { is_expected.to eq "https://github.com/NYULibraries/browbeat-screenshots/tree/jenkins-Some_Thing-2000" }
      end

      context "without jenkins_build_tag" do
        let(:build_tag){ nil }
        before{ allow(presenter).to receive(:jenkins_build_tag).and_return build_tag }
    ...
    describe "jenkins_env?" do
      subject{ presenter.jenkins_env? }
    ...
    describe "jenkins_build_tag" do
      subject{ presenter.jenkins_build_tag }
      ...
      context "with BUILD_TAG set" do
        ...
        it { is_expected.to eq build_tag }
      end

      context "without BUILD_TAG set" do
        it { is_expected.to eq nil }
      end
    end
  end
